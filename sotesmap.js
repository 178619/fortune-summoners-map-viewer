...
Object.freeze(SpriteSheetsIds)
Object.freeze(SpriteSheets)
class Map {
    constructor (arrayBuffer) {
        ...
        this.mapParallaxFront   = new  MapParallax(arrayBuffer, 0x000004, 0x06)
        this._off_0a            = new   Uint8Array(arrayBuffer, 0x00000A, 0x02)
        this._off_0c            = new   Uint8Array(arrayBuffer, 0x00000C, 0x04)
        this.mapParallaxBack    = new  MapParallax(arrayBuffer, 0x000010, 0x06)
        ...
        this.tileGraphics       = new TileGraphics(arrayBuffer, 0x000030, 0x140000)
        this.tilePhysics        = new  TilePhysics(arrayBuffer, 0x140030, 0x050000)
...
class TileGraphics {
    constructor (arrayBuffer, start, length) {
        ...
        this.tiles = new Array(parseInt(arrayBuffer.byteLength / TileGraphic.byteLength))
        for (let i=0; i<this.tiles.length; i++) {
            this.tiles[i] = new TileGraphic(arrayBuffer.slice(i*TileGraphic.byteLength, (i+1)*TileGraphic.byteLength))
        }
    }
    getTile(x, y, z) {
...
class TilePhysics {
    constructor (arrayBuffer, start, length) {
        arrayBuffer = arrayBuffer.slice(start, start+length)
        this._raw = arrayBuffer
        this.tiles = new Array(parseInt(arrayBuffer.byteLength / TilePhysic.byteLength))
        for (let i=0; i<this.tiles.length; i++) {
            this.tiles[i] = new TilePhysic(arrayBuffer.slice(i*TilePhysic.byteLength, (i+1)*TilePhysic.byteLength))
        }
    }
    getTile(x, y) {
...
Object.freeze(TileMaterial)
const displayData = (element, dataAsArrayBuffer) => {
    const map = new Map(dataAsArrayBuffer)
    displayDataFromMap(element, map)
    return map
}
const displayDataFromMap = (element, map) => {
    element.innerHTML = ''
    if (map.backgroundImage) {
        const background = document.createElement('div')
        background.classList.add('background')
        background.style.width = map.widthPixel / 100 + 'px'
        background.style.height = map.heightPixel / 100 + 'px'
        background.style.backgroundImage = 'url(images/'+SpriteSheetsIds[map.backgroundImage]+'.png)'
    ...
    if (map.mapParallaxFront.image) {
        ...
        parallaxFront.style.width = map.widthPixel / 100 + 'px'
        parallaxFront.style.height = map.heightPixel / 100 + 'px'
        // parallaxFront.style.backgroundPositionY = map.mapParallaxFront.offsetY + 'px'
        parallaxFront.style.backgroundImage = 'url(images/'+SpriteSheetsIds[map.mapParallaxFront.image]+'.png)'
    ...
    if (map.mapParallaxBack.image) {
        ...
        parallaxBack.style.width = map.widthPixel / 100 + 'px'
        parallaxBack.style.height = map.heightPixel / 100 + 'px'
        // parallaxBack.style.backgroundPositionY = map.mapParallaxBack.offsetY + 'px'
        parallaxBack.style.backgroundImage = 'url(images/'+SpriteSheetsIds[map.mapParallaxBack.image]+'.png)'
    ...
    layers.forEach((v, z)=>{
        v.classList.add('layer')
        v.style.zIndex = 1000+z
        for (let y=0;y<map.height;y++) {
            ...
            for (let x=0;x<map.width;x++) {
                ...
                if (t.spriteSheet) {
                    d.style.backgroundImage = 'url(images/'+SpriteSheetsIds[t.spriteSheet]+'.png)'
                    const s = SpriteSheets[SpriteSheetsIds[t.spriteSheet]]
